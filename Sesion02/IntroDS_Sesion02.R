...
setwd("~/Desktop/intro_datascience/Sesion02")
...
as.numeric(Sys.time()) #obtenemos el horario de ahora en forma numerica, por segundos desde 1970
#strptime(): le pasamos una feche en formato character, le pasamos el formato de fecha que queramos  y lo transforma a formato fecha en POSIXlt. 
...
# bicis$destinofecha - bicis%origenfecha #obtenemos el tiempo de uso en minutos, multiplicando por 60 lo pasamos a segundos. 
...
str(inc) 
hola = sort(table(inc$occupation))
...
inc$occupation_NA <- is.na(inc$occupation)


#Ej5: Reemplace los valores NA de occupation por el valor más frecuente de occupation. 
#consultar como llegue a que Prof-specialty es la ocupacion mas frecuente. 

# ifelse(is.na(inc$occupation) ==TRUE, "Prof-specialty" ,inc$occupation)


#Ej6: Arme un nuevo data.frame llamado inc_NA que contenga sólo la filas que tienen NA en native.country

inc_NA <- inc #creamos un dataset para modificarlo
inc_NA <- inc[is.na(inc$native.country),]
...
boxplot(data = bicis[bicis$tiempouso > 0 & bicis$tiempouso <120 ,],
        tiempouso ~ finDeSemana,
        ylab = "Tiempo usado (minutos)")
...
#graficamos horizontal cuando hay problemas con los labels. 
...
#hasta recien vimos como graficar la distribucion de una sola variable

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#lo que sigue es util para graficar como dos variables se distribuyen, y ver posibles asociaciones. 
plot(mtcars$mpg, mtcars$hp,
     main = "Relación entre hp y mpg",
     xlab = "Millas por galón",
     ylab = "Caballos de fueza", pch = 19, col = "orange")
#lo anterior grafica la nube de puntos de mog y hp.

abline(lm(hp ~ mpg, data = mtcars), col = "violet") #regresion lineal. 
...
par(mfrow=c(2,1)) #aca decimos que habra dos graficos en la ventana.
