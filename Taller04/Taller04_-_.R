...
# min_bound mínimo de documento en los que tienen que aparecer las palabras.
...
df <- readRDS("data.RDS") # Cargamos el dataset
...
#Intentaremos ver si la categoria del producto de la publicacion sirve como predictor.
# Por ejemplo, algunas publicaciones son de deportes;otras ropa;otras herramientas y construccion;otras computacion; etc.
df$full_name <- word(df$full_name, sep = "->") # Solo nos quedamos con la categoría principal (para poder hacer OHE)

#~~~~~~~~~~~~~
# One Hot Encoding Sparse: pasamos las variables tipo caracteres a formato factor. Tambien pasamos unas variables logicas a numericas. 
...
#---------------GRAFICOS Y TABLAS EXPLORATORIOS: 
...
# Proporción de conversion igual a uno e igual a cero para cada categoría de full_name:
prop.table(table(df$full_name, df$conversion), margin = 1 )

# Vemos cómo es la propocióm de conversion igual a uno y cero para las cuatro categorias de platform:
...
#~~~~~~~~~~~~~
# Bag of Words: Haremos analisis de sentimiento sobre los titulos de las publicaciones 
...
# Pasamos el dataframe a formato matriz rala: 
